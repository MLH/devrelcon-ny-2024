...
async function main() {
  ...
  scheduleSnapshot.forEach((doc) => {
    const timeslots = doc.data()['timeslots'] as
      | Array<{ sessions: Array<{ items: string[] }> }>
      | undefined;
    if (!timeslots) return;
  ...
  const speakerTalks = new Map<
    string,
    Array<{ title: string; tags: string[]; presentation?: string; videoId?: string }>
  >();
  ...
  speakersSnapshot.forEach((doc) => {
    ...
    const history = data['history'] as
      | Record<string, { talks?: Array<{ title: string }> }>
      | undefined;
    ...
    const currentTitles = currentTalks
      .map((t) => t.title)
      .sort()
      .join('|');
    const correctTitles = correctTalks
      .map((t) => t.title)
      .sort()
      .join('|');
    ...
    console.log(
      `  [rebuild] ${data['name']} — talks: ${currentTalks.length} → ${correctTalks.length}`,
    );
    if (!dryRun) {
      batch.update(doc.ref, {
