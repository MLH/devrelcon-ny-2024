...
async function main() {
  ...
  if (!dryRun && generatedSnapshot.size > 0) {
    ...
    for (let i = 0; i < generatedSnapshot.docs.length; i += batchSize) {
      ...
      await batch.commit();
      console.log(
        `  Deleted ${Math.min(i + batchSize, generatedSnapshot.size)} / ${generatedSnapshot.size}`,
      );
    }
  ...
  if (!dryRun) {
    ...
    console.log(
      '  Done! Cloud Function will regenerate generatedSpeakers, generatedSessions, and generatedSchedule.',
    );
  }
