...
async function main() {
  ...
  for (const [canonicalId, docs] of duplicates) {
    ...
    for (const [sessionId, sessionDoc] of sessionDocs) {
      ...
      const updated = [
        ...new Set(speakers.map((id) => (aliasIds.includes(id) ? canonicalId : id))),
      ];
      console.log(
        `    [session] ${sessionId}: speakers ${JSON.stringify(speakers)} → ${JSON.stringify(updated)}`,
      );
      if (!dryRun) {
  ...
  console.log(
    `Summary: ${mergeOps} merged, ${deleteOps} deleted, ${sessionUpdates} sessions updated`,
  );
